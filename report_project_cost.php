...
// Build query
// Project Cost = budget, Advance = approved/paid payment requests, Vouchers = invoices against those requests
$query = "SELECT 
    p.id,
...
$query .= " GROUP BY p.id, p.project_name, p.project_code, p.budget";
...
$current_page = 'report_project_cost';
$page_title = 'Project Cost Report';
$show_search = false;
...
<div class="page-content">
    <div class="page-header">
        <div class="page-title">
            ...
            <p>Project Cost, Advance and Voucher summary per project.</p>
        </div>
    ...
    <!-- Filter Bar -->
    <form method="GET" action="report_project_cost.php" style="display: flex; gap: 16px; align-items: flex-end; margin-bottom: 24px; flex-wrap: wrap;">
        ...
        <a href="report_project_cost.php" class="btn-secondary" style="height: 42px; display: flex; align-items: center; text-decoration: none;">
            Clear
        </a>
    </form>
                    ...
                    <?php if (count($records) > 0): ?>
                        <?php foreach ($records as $row): ?>
                            <tr>
                                ...
                                <td><span class="font-medium">₹<?php echo number_format($row['project_cost'], 2); ?></span></td>
                                <td><span class="font-medium">₹<?php echo number_format($row['total_advance'], 2); ?></span></td>
                                <td><span class="font-medium">₹<?php echo number_format($row['total_vouchers'], 2); ?></span></td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr>
...
<script>
    ...
    function exportReport(format) {
        ...
        window.location.href = 'handlers/export_project_cost_report.php?format=' + format + params;
    }
    ...
    function exportPDF() {
        ...
        doc.text('Project Cost Report', 14, 18);
        ...
        const headers = [['Project Name', 'Project Cost (Rs.)', 'Advance (Rs.)', 'Vouchers (Rs.)']];
        const rows = [];
        ...
        doc.save('Project_Cost_Report_' + new Date().toISOString().slice(0,10) + '.pdf');
    }
</script>
